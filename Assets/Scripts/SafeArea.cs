...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        {
            new Rect (0f, 102f / 2436f, 1f, 2334f/2436f),  // Portrait
            // new Rect (0f, 102f / 2436f, 1f, 2202f / 2436f),  // Portrait
            new Rect (132f / 2436f, 63f / 1125f, 2172f / 2436f, 1062f / 1125f),  // Landscape
            new Rect (0f, 0f, 1f, 2202f / 2436f),  // Portrait
            new Rect (132f / 2436f, 0f, 2172f / 2436f, 1f)  // Landscape
        };
        ...
        Rect[] NSA_iPhoneXsMax = new Rect[]
        {
            ...
            new Rect (0f, 0f, 1f, 2454f / 2688f),  // Portrait
            new Rect (132f / 2688f, 0f, 2424f / 2688f, 1f)  // Landscape
        };

        // Rect[] NSA_iPadPro = new Rect[]
        // {
        //     new Rect (0f, 1f, 1f, 2789f / 2388f),  // Portrait
        ...
        [SerializeField] bool IgnoreYBottomIOS = false;  // Conform to screen safe area on X-axis (default true, disable to ignore)
        [SerializeField] bool IgnoreOffset = false;  // Conform to screen safe area on X-axis (default true, disable to ignore)
        [SerializeField] bool Logging = false;  // Conform to screen safe area on Y-axis (default true, disable to ignore)

        // callback
        [Serializable]
        public class ApplySafeAreaDelegate : UnityEvent { } // code, status, data, baseData
        public ApplySafeAreaDelegate OnApplySafeArea = new ApplySafeAreaDelegate();

        public void AddListener(UnityAction action)
        {
            OnApplySafeArea.AddListener(action);
        }

        public void RemoveListener(UnityAction action)
        {
            OnApplySafeArea.RemoveListener(action);
        }
        ...
        void OnDestroy()
        {
            // VKSdk.VKSafeArea.RemoveArea(this);
        }
                ...
                switch (Sim)
                {
                    case SimDevice.iPhoneX:
                        if (Screen.height > Screen.width)  // Portrait
                            nsa = IgnoreYBottomIOS ? NSA_iPhoneX[2] : NSA_iPhoneX[0];
                        else  // Landscape
                            nsa = IgnoreYBottomIOS ? NSA_iPhoneX[3] : NSA_iPhoneX[1];
                        break;
                    case SimDevice.iPhoneXsMax:
                        if (Screen.height > Screen.width)  // Portrait
                            nsa = IgnoreYBottomIOS ? NSA_iPhoneXsMax[2] : NSA_iPhoneXsMax[0];
                        else  // Landscape
                            nsa = IgnoreYBottomIOS ? NSA_iPhoneXsMax[3] : NSA_iPhoneXsMax[1];
                        break;
        ...
        void ApplySafeArea(Rect r)
        {
            ...
            if (Screen.width > 0 && Screen.height > 0)
            {
                ...
                // Debug.Log("r.position " + r.position.x +", " + r.position.y);
                // Debug.Log("r.size " + r.size.x +", " + r.size.y);
                Vector2 anchorMin = r.position + Vector2.zero;//(IgnoreOffset ? Vector2.zero : VKSdk.VKSafeArea.offset);
